            ...
            for(int d = 0;d < DIG;d++) {
                ...
                if(d > char2int(l[i])) {
                    for(int dprev = 0;dprev < DIG;dprev ++) {
                        if(k > 0)
                            dpg[i][k][d] += dpl[i - 1][k - 1][dprev] + dpg[i - 1][k - 1][dprev];
                    }
                }
                ...
                if(d > char2int(l[i])) {
                    ...
                    for(int dprev = 0;dprev < DIG;dprev++) {
                        int k0 = (d != 0 ? k - 1 : k);
                        if(k0 >= 0)
                            dpl[i][k][d] += dpl[i - 1][k0][dprev] + dpg[i - 1][k0][dprev];
                    }
                }
            }
        }
...
void solve() {
    ...
    for(int i = n - 2;i >= 0;i--) {
        ...
        if(isFrontEqual) {
            ...
            for(int d = 0;d < char2int(r[i]);d++)
                ans += dpg[i][kFront][d] + dpl[i][kFront][d];
        }
