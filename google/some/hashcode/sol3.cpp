...
void read()
{
    ...
    for(int i = 0;i < l;i++)
    {
        ...
        for(int j = 0;j < nb;j++)
        {
            ...
            scanf("%d",&book_id);
            bk[i].insert(book_id);
            sum_score[i] += s[book_id];
            book_to_lib[book_id].push_back(i);
        }
    }
}

int alpha = 1, beta = 0, gamma = 0;

long long compute_term(int lib)
{
    return alpha * sum_score[lib] - beta * sign_lib[lib] + gamma * num_bk_max[lib];
}
...
void solve()
{
    ...
    for(int i = 0;i < l;i++)
    {
        ...
        for(int j = 0;j < l;j++)
        {
            if(!signedup[j] && (max_lib == -1 || compute_term(max_lib) < compute_term(j)))
                max_lib = j;
        }
